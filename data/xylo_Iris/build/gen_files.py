...
BLOCK_TAG_PATH = "data/xylo_iris/tags/blocks"
FUNCTION_PATH = "data/xylo_iris/functions/get_hitbox"
...
if __name__ == "__main__":
    ...
    for group in block_shape_groups:
        ...
        block_id = ("#xylo_iris:shape_groups/" + get_representative(group)) if len(group)>1 else namespaced_representative
        ...
        with open(mcfunction_file_path, mode='w') as mcfunction_file:
            for state in block_data[namespaced_representative]["states"]:
                ...
                data_command = f"data modify storage xylo_iris:data Shape set value {snbt}"
    ...
    for i in range(PARTITION_SUBSETS):
        ...
        for group in block_shape_groups[i*groups_per_tag : (i+1)*groups_per_tag]:
            if len(group) > 1:
                ...
                tag_values.append(f"#xylo_iris:shape_groups/{repr}")
                commands.append(f"execute if block ~ ~ ~ #xylo_iris:shape_groups/{repr} run function xylo_iris:get_hitbox/block/shape_groups/{repr}")
            else:
                tag_values.append(group[0])
                commands.append(f"execute if block ~ ~ ~ {group[0]} run function xylo_iris:get_hitbox/block/shape_groups/{get_representative(group)}")
    ...
    with open(f"{FUNCTION_PATH}/block.mcfunction", mode='w') as function_file:
        ...
        HEADER = """#> xylo_iris:get_hitbox/block
#
...
"""
        ...
        commands = []
        for i in range(PARTITION_SUBSETS):
            commands.append(f"execute if block ~ ~ ~ #xylo_iris:tree/{i} run return run function xylo_iris:get_hitbox/block/tree/{i}")
